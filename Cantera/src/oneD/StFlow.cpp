    ...
    m_tfixed(Undef)
{
    ...
    m_creation.resize(m_nsp,m_points, 0.0);
    m_destruction.resize(m_nsp,m_points, 0.0);

    kc_t.resize(250,m_points, 0.0);
    Delta_Gibbs_t.resize(250,m_points, 0.0);
    Delta_Enthalpy_t.resize(250,m_points, 0.0);
    Delta_Entropy_t.resize(250,m_points, 0.0);
    Delta_SS_Gibbs_t.resize(250,m_points, 0.0);
    Delta_SS_Enthalpy_t.resize(250,m_points, 0.0);
    Delta_SS_Entropy_t.resize(250,m_points, 0.0);
    Rates_Progress_Fwd_t.resize(250,m_points, 0.0);
    Rates_Progress_Rev_t.resize(250,m_points, 0.0);
    Rate_Constants_Fwd_t.resize(250,m_points, 0.0);
    Rate_Constants_Rev_t.resize(250,m_points, 0.0);
...
void StFlow::resize(size_t ncomponents, size_t points)
{
    ...
    m_cv.resize(m_points, 0.0);
    m_Es.resize(m_points, 0.0);
    m_Hs.resize(m_points, 0.0);
...
void StFlow::updateProperties(size_t jg, double* x, size_t jmin, size_t jmax)
{
    ...
    //Nicolas Jaouen, update the constants for the calculation of
    //the reaction rates
    //Adapted for Cantera 2.4 by Andrea Seltz - Jan 19
    updateReactionConstants(x, j0, j1);
}
