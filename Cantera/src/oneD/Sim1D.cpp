...
void Sim1D::solve(int loglevel, bool refine_grid)
{
    ...
    while (new_points > 0) {
        ...
        while (!ok) {
            ...
            if (loglevel > 1)
              debuglog("Attempt Newton solution of steady-state problem...", 1);
            ...
            if (status == 0) {
                ...
                debuglog("    failure. \n", loglevel);
                cout << "    failure. " << endl;
                if (istep>m_max_isteps)
                {
                   ok = true;
                   m_newton_failed = true;
                }
                if (loglevel > 6) {
                    save("debug_sim1d.xml", "debug",
...
int Sim1D::refine(int loglevel)
{
    ...
    for (size_t n = 0; n < nDomains(); n++) {
        ...
        ianalyze = r.analyze(d.grid().size(),  DATA_PTR(d.grid()), &m_x[start(n)]);
        if (ianalyze < 0) {
            return ianalyze;
        }
...
double Sim1D::remesh(int loglevel)
{
    vector_fp znew, xnew;
    vector_int dsize;
    ...
    for (size_t n = 0; n < nDomains(); n++)
    {
       ...
       // procede the remeshing
       //doublereal this_distance = r.remeshFromSolution(d.grid().size(), DATA_PTR(d.grid()), m_x.data() + start(n));
       doublereal this_distance = r.remeshFromSolution(d.grid().size(), DATA_PTR(d.grid()), &m_x[start(n)]);
       ...
       for (int m = 0; m < np_new; m++)
       {
          ...
          if (np_new>1)
          {
             int m_old = r.indxtp(np_old,znew_curr,DATA_PTR(d.grid()));
             double z0_old = d.grid(m_old  );
             double z1_old = d.grid(m_old+1);
             for (unsigned int i = 0; i < d.nComponents(); i++)
             {
